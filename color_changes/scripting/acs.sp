	...
	if(action == MenuAction_Select)	{
		...
		if (StrEqual(menuInfo, MMC_ITEM_IDONTCARE_TEXT, false)) {
			...
			if (StrEqual(menuInfo, MMC_ITEM_ALLMAPS_TEXT, false)) {
				...
				if (IsVoteInProgress()) {
					ReplyToCommand(client, "{blue}[{default}ACS{blue}]\x04 %t", "Vote in Progress");
					return 0;
				}
...
public int ChampVoteHandler(Menu menu, MenuAction action, int param1, int param2) {
	if (action == MenuAction_Display) {
		...
		CPrintToChatAll("{blue}[{default}ACS{blue}]\x04 %t", "No Votes Cast");
		...
		if (percent < limit) {
			...
			CPrintToChatAll("{blue}[{default}ACS{blue}]\x04 %t  \x01[%d,%d,%d]", "Vote Failed", RoundToNearest(100.0*limit), RoundToNearest(100.0*percent), playerCount, yesVotes, noVotes, abstention);
		} else {
			CPrintToChatAll("{blue}[{default}ACS{blue}]\x04 %t  \x01[%d,%d,%d]", "Vote Successful", RoundToNearest(100.0*percent), playerCount, yesVotes, noVotes, abstention);
			...
			if (mapIndex < 0) {
				...
				for (int client = 1; client <= MaxClients; client++) {
					if (IsClientInGame(client)) {
						...
						Format(colorizedName, sizeof(colorizedName), "{olive}%s{default}", localizedName);
						CPrintToChat(client,"%t", "Mission is now winning the vote", colorizedName);
					}
				}			
			} else {
				...
				for (int client = 1; client <= MaxClients; client++) {
					if (IsClientInGame(client)) {
						LMM_GetMapLocalizedName(g_iGameMode, missionIndex, mapIndex, localizedName, sizeof(localizedName), client);
						Format(colorizedName, sizeof(colorizedName), "{olive}%s{default}", localizedName);
						CPrintToChat(client,"%t", "Map is now winning the vote", colorizedName);
					}
				}
			}
			
			Format(colorizedName, sizeof(colorizedName), "{olive}%s{default}", mapName);
			CPrintToChatAll("%t", "Changing map", colorizedName);
...
public void CVarChange_Voting(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	if (StringToInt(strNewValue) == 1) {
		...
		CPrintToChatAll("[ACS] ConVar changed: Voting System ENABLED");
	} else {
		PrintToServer("[ACS] ConVar changed: Voting System DISABLED");
		CPrintToChatAll("[ACS] ConVar changed: Voting System DISABLED");
	}
}
...
public void CVarChange_NewVoteWinnerSound(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	if (StringToInt(strNewValue) == 1) {
		...
		CPrintToChatAll("[ACS] ConVar changed: New vote winner sound ENABLED");
	} else {
		PrintToServer("[ACS] ConVar changed: New vote winner sound DISABLED");
		CPrintToChatAll("[ACS] ConVar changed: New vote winner sound DISABLED");
	}
}
...
public void CVarChange_VotingAdMode(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	switch(StringToInt(strNewValue)) {
		case 0:	{
			...
			CPrintToChatAll("[ACS] ConVar changed: Voting display mode: DISABLED");
		}
		case 1:	{
			PrintToServer("[ACS] ConVar changed: Voting display mode: HINT TEXT");
			CPrintToChatAll("[ACS] ConVar changed: Voting display mode: HINT TEXT");
		}
		case 2:	{
			PrintToServer("[ACS] ConVar changed: Voting display mode: CHAT TEXT");
			CPrintToChatAll("[ACS] ConVar changed: Voting display mode: CHAT TEXT");
		}
		case 3:	{
			PrintToServer("[ACS] ConVar changed: Voting display mode: OPEN VOTE MENU");
			CPrintToChatAll("[ACS] ConVar changed: Voting display mode: OPEN VOTE MENU");
		}
	}
}
...
public void CVarChange_VotingAdDelayTime(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	if (fDelayTime > 0.1)
	{
		...
		CPrintToChatAll("[ACS] ConVar changed: Voting advertisement delay time changed to %f", fDelayTime);
	}
	else
	{
		PrintToServer("[ACS] ConVar changed: Voting advertisement delay time changed to 0.1");
		CPrintToChatAll("[ACS] ConVar changed: Voting advertisement delay time changed to 0.1");
	}
}
...
public void CVarChange_NewMapAdMode(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	switch(StringToInt(strNewValue)) {
		case 0:	{
			...
			CPrintToChatAll("[ACS] ConVar changed: Next map advertisement display mode: DISABLED");
		}
		case 1:	{
			PrintToServer("[ACS] ConVar changed: Next map advertisement display mode: HINT TEXT");
			CPrintToChatAll("[ACS] ConVar changed: Next map advertisement display mode: HINT TEXT");
		}
		case 2:	{
			PrintToServer("[ACS] ConVar changed: Next map advertisement display mode: CHAT TEXT");
			CPrintToChatAll("[ACS] ConVar changed: Next map advertisement display mode: CHAT TEXT");
		}
	}
}
...
public void CVarChange_NewMapAdInterval(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	if (fDelayTime > 60.0) {
		...
		CPrintToChatAll("[ACS] ConVar changed: Next map advertisement interval changed to %f", fDelayTime);
	} else {
		PrintToServer("[ACS] ConVar changed: Next map advertisement interval changed to 60.0");
		CPrintToChatAll("[ACS] ConVar changed: Next map advertisement interval changed to 60.0");
	}
}
...
public void CVarChange_MaxFinaleFailures(Handle hCVar, const char[] strOldValue, const char[] strNewValue) {
	...
	if (iMaxFailures > 0) {
		...
		CPrintToChatAll("[ACS] ConVar changed: Max Coop finale failures changed to %f", iMaxFailures);
	} else {
		PrintToServer("[ACS] ConVar changed: Max Coop finale failures changed to 0");
		CPrintToChatAll("[ACS] ConVar changed: Max Coop finale failures changed to 0");
	}
}
/*======================================================================================
#################                     E V E N T S                      #################
...
public Action Event_RoundEnd(Handle hEvent, const char[] strName, bool bDontBroadcast) {
	// CPrintToChatAll("{blue}[{default}ACS{blue}]\x04 Event_RoundEnd");
...
public Action Event_FinaleWin(Handle hEvent, const char[] strName, bool bDontBroadcast) {
	// CPrintToChatAll("{blue}[{default}ACS{blue}]\x04 Event_FinaleWin");
		...
		if(StrEqual(strCurrentMap, mapName, false)) {
			for (int client = 1; client <= MaxClients; client++) {
				if (IsClientInGame(client)) {
					ACS_GetLocalizedMissionName(g_iGameMode, cycleIndex, client, localizedName, sizeof(localizedName));
					Format(colorizedname, sizeof(colorizedname), "{olive}%s{default}", localizedName);
					CPrintToChat(client, "%t", "Campaign finished", colorizedname);
				}
			}
				...
				if(IsMapValid(mapName)) {
					for (int client = 1; client <= MaxClients; client++) {
						if (IsClientInGame(client)) {
							...
							Format(colorizedname, sizeof(colorizedname), "{olive}%s{default}", localizedName);
							CPrintToChat(client, "%t", "Switching to the vote winner", colorizedname);
						}
					}
			...
			if(IsMapValid(mapName)) {
				for (int client = 1; client <= MaxClients; client++) {
					if (IsClientInGame(client)) {
						ACS_GetLocalizedMissionName(g_iGameMode, cycleIndex, client, localizedName, sizeof(localizedName));
						Format(colorizedname, sizeof(colorizedname), "{olive}%s{default}", localizedName);
						CPrintToChat(client, "%t", "Switching campaign to", colorizedname);
					}
				}
				...
				if(IsMapValid(mapName)) {
					for (int client = 1; client <= MaxClients; client++) {
						if (IsClientInGame(client)) {
							LMM_GetMapLocalizedName(g_iGameMode, missionIndex, mapIndex, localizedName, sizeof(localizedName), client);
							Format(colorizedname, sizeof(colorizedname), "{olive}%s{default}", localizedName);
							CPrintToChat(client, "%t", "Switching map to", colorizedname);
						}
					}	
			...
			if(g_iGameMode == GAMEMODE_SCAVENGE) {
				...
				for (int client = 1; client <= MaxClients; client++) {
					if (IsClientInGame(client)) {
						LMM_GetMapLocalizedName(g_iGameMode, missionIndex, mapIndex, localizedName, sizeof(localizedName), client);
						Format(colorizedName, sizeof(colorizedName), "{olive}%s{default}", localizedName);
						CPrintToChat(client, "%t", "The next map is currently", colorizedName);
					}
				}
			} else {
				for (int client = 1; client <= MaxClients; client++) {
					if (IsClientInGame(client)) {
						LMM_GetMissionLocalizedName(g_iGameMode, g_iWinningMapIndex, localizedName, sizeof(localizedName), client);
						Format(colorizedName, sizeof(colorizedName), "{olive}%s{default}", localizedName);
						CPrintToChat(client, "%t", "The next campaign is currently", colorizedName);
					}
				}
			}
		}
...
public Action MapVote(int iClient, int args) {
	if(!g_hCVar_VotingEnabled.BoolValue) {
		ReplyToCommand(iClient, "{blue}[{default}ACS{blue}]\x01 %t", "Voting is disable");
		return;
	}
	
	if(!OnFinaleOrScavengeMap()) {
		CPrintToChat(iClient, "%t", "Voting is not available");
		return;
	}
...
public Action DisplayCurrentVotes(int iClient, int args) {
	...
	if(!OnFinaleOrScavengeMap()) {
		CPrintToChat(iClient, "{blue}[{default}ACS{blue}]\x01 %t", "Voting is not available");
		return;
	}
	...
	if(g_iWinningMapIndex >= 0) {
		...
		if(g_iGameMode == GAMEMODE_SCAVENGE) {
			...
			Format(colorizedName, sizeof(colorizedName), "{olive}%s{default}", localizedName);
			ReplyToCommand(iClient,"%t", "Map is now winning the vote", colorizedName);	
		} else {
			LMM_GetMissionLocalizedName(g_iGameMode, g_iWinningMapIndex, localizedName, sizeof(localizedName), iClient);
			Format(colorizedName, sizeof(colorizedName), "{olive}%s{default}", localizedName);
			ReplyToCommand(iClient,"%t", "Mission is now winning the vote", colorizedName);
		}
	} else {
		ReplyToCommand(iClient, "%t", "No one has voted yet");	
	}
	...
	for(int iClient = 1;iClient <= MaxClients; iClient++) {
		if(
			...
			switch(g_hCVar_VotingAdMode.IntValue) {
				...
				case DISPLAY_MODE_CHAT: CPrintToChat(iClient, "%t", "Map vote advertise", "\x04!mapvote\x05", "\x04!mapvotes\x05");
			}
