...
{
	name = "L4D2 Tank Swap",
	author = "AtomicStryker [Mis edit]",
	description = " Allows a primary Tank Player to surrender control to one of his teammates, or admins to take it anytime (hides tankhud)",
	version = "1.0",
...
public Action:TS_DisplayNotificationToTank(Handle:timer)
{
	...
	CPrintToChat(primaryTankPlayer, "{red}[{default}!{red}]{default} Press {olive}F {default}to surrender tank control\n{red}[{default}!{red}]{default} You have {olive}%i {default}secconds to do it", RoundFloat(SurrenderTimeLimit));
	return Plugin_Stop;
}
...
public TS_MenuCallBack(Handle:menu, MenuAction:action, param1, param2)
{
    ...
    else if (action == MenuAction_Select)
    {
        ...
        if (!choice)
        {
            ...
            CPrintToChatAll("{red}[{default}!{red}] {default}Tank Control surrendered {olive}%N", choice);
...
stock DebugPrintToAll(const String:format[], any:...)
{
	...
	#if TEST_DEBUG
	CPrintToChatAll("[TANKVOTE] %s", buffer);
